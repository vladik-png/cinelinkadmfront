...
const Analytics: React.FC = () => {
  ...
  const fetchAllMetrics = async () => {
    try {
      ...
      console.error("Global metrics fetching error:", err);
    }
  };
        ...
        <div className="mb-10 border-b border-slate-200 pb-8 flex justify-between items-end">
          ...
          {selectedNode && (
            <button 
              ...
              className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition shadow-md border border-slate-700"
            >
          ...
          {displayedNodes.map(nodeId => {
            ...
            return (
              <div 
                ...
                className={`bg-white rounded-[2rem] border ${selectedNode ? 'border-blue-300 shadow-md ring-4 ring-blue-50' : 'border-slate-200 shadow-sm'} overflow-hidden cursor-pointer hover:border-blue-300 transition-all group`}
              >
                <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                  ...
                  <div className="flex items-center gap-6">
                    {viewMode === 'combined' && (
                      <div className="hidden lg:flex gap-8 text-[11px] uppercase font-black tracking-widest">
                          <div className="text-blue-500">CPU: {latest?.cpu}%</div>
                          <div className="text-rose-500">TEMP: {latest?.temp}°C</div>
                ...
                <div className="p-10">
                  {viewMode === 'combined' ? (
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={history} margin={{ left: -10, bottom: 20 }}>
                          <defs>
                            ...
                            <linearGradient id="colorTemp" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f43f5e" stopOpacity={0.1}/><stop offset="95%" stopColor="#f43f5e" stopOpacity={0}/></linearGradient>
                          ...
                          <Area name="Temp (°C)" type="monotone" dataKey="temp" stroke="#f43f5e" strokeWidth={3} fill="url(#colorTemp)" />
                          <Area name="RAM (%)" type="monotone" dataKey="ram" stroke="#a855f7" strokeWidth={3} fill="transparent" />
                          <Area name="Disk (%)" type="monotone" dataKey="disk" stroke="#f97316" strokeWidth={3} fill="transparent" />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <MiniChart title="Temperature" value={`${latest?.temp}°C`} data={history} dataKey="temp" color="#f43f5e" icon={<Thermometer size={14}/>} />
                      <MiniChart title="RAM Usage" value={`${latest?.ram}%`} data={history} dataKey="ram" color="#a855f7" icon={<Activity size={14}/>} />
                      <MiniChart title="Network Ping" value={`${latest?.ping}MS`} data={history} dataKey="ping" color="#10b981" icon={<Clock size={14}/>} yDomain={['auto', 'auto']} />
...
const MiniChart = ({ title, value, data, dataKey, color, icon, isStep, yDomain = [0, 100] }: any) => (
  <div className="bg-slate-50/50 p-6 rounded-[1.5rem] border border-slate-100 cursor-default" onClick={(e) => e.stopPropagation()}>
    ...
    <div className="h-[180px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={data} margin={{ left: -20, bottom: 20 }}>
          ...
          <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 10}} dy={10} />
          <YAxis domain={yDomain} axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 10}} />
